...
export const addPackageValidator = [
    check('shortTitle').trim().not().isEmpty().withMessage('shortTitle is required').isString().isLength({ min: 2 }).withMessage('shortTitle min length is 2'),
    check('shortDescription').trim().not().isEmpty().withMessage('shortDescription is required').isString().isLength({ min: 2 }).withMessage('shortDescription min length is 2'),
